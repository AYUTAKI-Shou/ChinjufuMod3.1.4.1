...
public class BlockKit_frypan1 extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (heldItem == null) {
			...
			if (playerIn instanceof EntityPlayer)
	    		((EntityPlayer) playerIn).inventory.clearMatchingItems(Item.getItemFromBlock(ChinjufuModFoodBlocks.FRYPAN), -1, 1, null);
			worldIn.setBlockState(pos, ASDecoModKitchen.KIT_FRYPAN2.getDefaultState()
					.withProperty(FACING, state.getValue(FACING)));
		}
		return true;
	}
