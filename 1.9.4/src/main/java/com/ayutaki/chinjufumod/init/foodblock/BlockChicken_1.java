	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
    	...
    	Item item = heldItem.getItem();
       	if (item == TTimeItems.Item_SARA) {
       		...
       		/*手持ちアイテムの削除パターンは巧くいかないため clearMatchingItems で*/
			if (playerIn instanceof EntityPlayer)
    			((EntityPlayer) playerIn).inventory.clearMatchingItems(TTimeItems.Item_SARA, -1, 1, null);
    		if (playerIn instanceof EntityPlayer)
				((EntityPlayer) playerIn).inventory
				.addItemStackToInventory(new ItemStack(Item.getItemFromBlock(ChinjufuModFoodBlocks.CHICKENB_1), 1));

			worldIn.setBlockState(pos, ChinjufuModFoodBlocks.CHICKEN_2
