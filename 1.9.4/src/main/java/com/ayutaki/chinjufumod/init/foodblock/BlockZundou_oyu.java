...
public class BlockZundou_oyu extends BlockFacingSapo {
	...
	private static final AxisAlignedBB BOUNDING_BOX_NORTH = CollisionHelper.getBlockBounds(EnumFacing.NORTH, 0.25, 0.0, 0.25, 0.75, 0.5625, 0.75);
	private static final AxisAlignedBB BOUNDING_BOX_EAST = CollisionHelper.getBlockBounds(EnumFacing.EAST, 0.25, 0.0, 0.25, 0.75, 0.5625, 0.75);
	private static final AxisAlignedBB BOUNDING_BOX_SOUTH = CollisionHelper.getBlockBounds(EnumFacing.SOUTH, 0.25, 0.0, 0.25, 0.75, 0.5625, 0.75);
	private static final AxisAlignedBB BOUNDING_BOX_WEST = CollisionHelper.getBlockBounds(EnumFacing.WEST, 0.25, 0.0, 0.25, 0.75, 0.5625, 0.75);
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		else if (heldItem.getItem() == TTimeItems.PASTA_nama) {

			/*アイテム変換を伴わないため clearMatchingItems で*/
			if (playerIn instanceof EntityPlayer)
	    		((EntityPlayer) playerIn).inventory.clearMatchingItems(TTimeItems.PASTA_nama, -1, 1, null);
			worldIn.setBlockState(pos, ChinjufuModFoodBlocks.PASTANABE_full
					.getDefaultState().withProperty(FACING, state.getValue(FACING)));
		}
		return true;
	}
