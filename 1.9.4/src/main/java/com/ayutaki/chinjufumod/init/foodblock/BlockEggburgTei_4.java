...
public class BlockEggburgTei_4 extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if (!playerIn.capabilities.allowEdit) {
		...
		else if (playerIn instanceof EntityLivingBase) {
			...
			return worldIn.setBlockState(pos, ChinjufuModFoodBlocks.EGGBURGSET_kara.getDefaultState().withProperty(FACING, state.getValue(FACING)));
		}
		return true;
	}
