...
public class BlockKit_tcupB extends BlockFacingSapo {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn,
		...
		if (!playerIn.capabilities.allowEdit) {
    		...
    		worldIn.setBlockState(pos, ASDecoModKitchen.KIT_TCUPA.getDefaultState().withProperty(FACING, state.getValue(FACING)));

		}

		else if (heldItem.getItem() == Item.getItemFromBlock(ChinjufuModFoodBlocks.TEAPOT_kara)) {

			if (playerIn instanceof EntityPlayer)
	    		((EntityPlayer) playerIn).inventory.clearMatchingItems(Item.getItemFromBlock(ChinjufuModFoodBlocks.TEAPOT_kara), -1, 1, null);
			worldIn.setBlockState(pos, ASDecoModKitchen.KIT_TCUPC
